...
export default function(ctx) {
  ...
  events.mousedrag = function(event) {
    events.drag(event, (endInfo) => { return !isClick(mouseDownInfo, endInfo); });
  };

  events.touchdrag = function(event) {
    events.drag(event, (endInfo) => { return !isTap(touchStartInfo, endInfo); });
  };
  ...
  // 8 - Backspace
  // 46 - Delete
  const isKeyModeValid = (code) => { return !(code === 8 || code === 46 || code >= 48 && code <= 57); };
  ...
  events.data = function(event) {
    if (event.dataType === 'style') {
      ...
      const hasLayers = options.styles.some((style) => { return map.getLayer(style.id); });
      if (!hasLayers) {
        setup.addLayers();
        store.setDirty();
        store.render();
